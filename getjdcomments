...
class JDCommentsCrawler:
    ...
    def concatLinkParam(self):
        self.locationUrl = self.locationLink+'?'+self.paramDict2Str(self.paramValue)+'isShadowSku=0&fold=1&page=1'
    ...
    def save_csv(self,df,p):
        # 保存文件
        df.to_csv(path_or_buf = 'E:\\pachong1\\jd_%d.csv'%p,encoding='gbk')
    ...
    def crawler(self):
        ...
        for p in range(self.page):
            ...
            self.save_csv(df,p)
            dfs.append(df)
            time.sleep(random.randint(33,90))
